...
class MiniAnalysis:
    def __init__(self, dataplan:object):
        """
    ...
    # from acq4 functions:
    def measure_baseline(self, data:np.ndarray, threshold:float=2.0, iterations:int=2):
    ...
    def analyze_all(self, fofilename:str, check:bool=False, mode:str="aj", engine:str="cython")->None:
        """
        ...
        if not check:
            fout = "summarydata_{0:s}_{1:s}_{2:s}.p".format(
                self.datasource, self.filterstring, mode
            )
            fh = open(fout, "wb")
    ...
    def analyze_one(
        ...
        for nprot, dprot in enumerate(mousedata["prots"]):
            ...
            if ext != ".mat":
                ...
                dt = 1000.0 * acqr.sample_interval  # in sec... so convert to msec
            else:
                print(".mat")
                DF = EP.MatdatacRead.DatacFile(fn)
    ...
    def analyze_block_traces(
        ...
        self.ax0.set_xlim(-1500.0, np.max(self.acqr.time_base) * 1000.0)
        nused = 0
        ...
        for i in tracelist:
            ...
            data[i] = DF.SignalFilter_LPFButter(
                data[i], 2800.0, 1000.0 / dt, NPole=4
            )  # always LPF data
            #            frsfilt, freqsf = PU.pSpectrum(dfilt, samplefreq=1000./dt)

            if mode == "aj":
                aj.deconvolve(
                    data[i], thresh=float(mousedata["thr"]), llambda=10.0, order=order
                )
            else:
                aj.cbTemplateMatch(
                ...
                [
                    {
                        ...
                        "avg": aj.avgevent,
                        "fit": {
                            "amplitude": aj.Amplitude,
                            "tau1": aj.tau1,
                            "tau2": aj.tau2,
                            "risepower": aj.risepower,
                        },
                        "best_fit": aj.avg_best_fit,
                        "risetenninety": aj.risetenninety,
                        "decaythirtyseven": aj.decaythirtyseven,
                        "Qtotal": aj.Qtotal,
                    }
                ]
            )
            ...
            self.ax0.plot(
                aj.timebase[aj.smpkindex],
                data[i][aj.smpkindex] + yp,
                "ro",
                markersize=1.75,
                rasterized=self.rasterize,
            )
    ...
    def plot_individual_events(
        self, fit_err_limit:float=1000.0, tau2_range:float=2.5, title:str="", pdf:object=None
    )->None:
        P = PH.regular_grid(
            3,
            3,
    ...
    def plot_all_events_and_fits(self)->None:
        P3 = PH.regular_grid(
            1,
            5,
            order="columns",
    ...
    def plot_hists(self)->None:  # generate histogram of amplitudes for plots
        histBins = 50
        nsamp = 1
        ...
        print(
            "    ampgamma: mean: {0:.4f} gamma:{1:.4f}  loc: {1:.4f}  scale: {3:.4f} midpoint: {4:.4f}".format(
